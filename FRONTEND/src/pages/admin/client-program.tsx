...
export default function AdminClientProgramPage() {
    ...
    const fetchData = async () => {
        try {
            setLoading(true);
            const [clientRes, workoutRes, mealRes, planRes] = await Promise.all([
                userApi.adminGetClientById(clientId!),
                workoutApi.getAll(),
                mealApi.getAll(),
                weeklyPlanApi.getAll(clientId)
            ]);
    ...
    const handleSave = async () => {
        try {
            setSaving(true);
            await weeklyPlanApi.updateByDay(selectedDay, currentPlan, clientId);
            toast.success(`Plan for ${selectedDay} updated successfully for ${client?.userName || 'Client'}`);
            
            // Re-fetch only plans to update local state without full reload
            const planRes = await weeklyPlanApi.getAll(clientId);
            setWeeklyPlans(planRes.data || []);
        } catch (error: any) {
            ...
            <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="flex items-center gap-4">
                    <Button 
                        ...
                        className="rounded-full h-12 w-12 bg-muted/50 hover:bg-muted"
                    >
                    ...
                    <div>
                        <h1 className="text-3xl font-black tracking-tight flex items-center gap-3">
                            ...
                            {client?.userName || client?.name || 'Client'}'s Weekly Program
                        </h1>
                        <p className="text-muted-foreground font-medium">
                            Customizing elite protocols and nutrition strategy.
                        </p>
                    </div>
                </div>
                <Button 
                    onClick={handleSave} 
                    disabled={saving} 
                    className="rounded-full px-8 font-bold gap-2 shadow-lg shadow-primary/20 h-12"
                >
                    ...
                    {saving ? "Deploying..." : "Save Daily Strategy"}
                </Button>
            </header>
                    ...
                    <CardContent className="space-y-3">
                        <div className="max-h-[500px] overflow-y-auto pr-2 custom-scrollbar space-y-2">
                            {workouts.length > 0 ? workouts.map((w) => (
                                ...
                                <p className="text-center py-10 text-muted-foreground text-sm italic">No workouts available. Create some first.</p>
                            )}
                        </div>
                    </CardContent>
                </Card>
                    ...
                    <CardContent className="space-y-3">
                        <div className="max-h-[500px] overflow-y-auto pr-2 custom-scrollbar space-y-2">
                            {meals.length > 0 ? meals.map((m) => (
                                ...
                                <p className="text-center py-10 text-muted-foreground text-sm italic">No meals available. Create some first.</p>
                            )}
                        </div>
                    </CardContent>
                </Card>
                    ...
                    <CardContent className="space-y-6">
                        <div className="space-y-2">
                            ...
                            <textarea
                                ...
                                placeholder="Enter specific instructions for this client on this day..."
