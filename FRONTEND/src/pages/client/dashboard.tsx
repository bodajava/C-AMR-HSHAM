...
export default function DashboardPage() {
  ...
  const [selectedDay, setSelectedDay] = useState<string>(new Date().toLocaleDateString('en-US', { weekday: 'long' }).toLowerCase());
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const [metricsRes, planRes, allPlansRes] = await Promise.all([
          metricsApi.getMetrics(targetClientId || undefined),
          weeklyPlanApi.getByDay(selectedDay, targetClientId || undefined),
          weeklyPlanApi.getAll(targetClientId || undefined)
        ]);
        setMetrics(metricsRes.data);
        setTodayPlan(planRes.data);
        setWeeklyPlans(allPlansRes.data || []);
      } catch (error) {
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div className="space-y-1">
          ...
          <p className="text-muted-foreground text-lg">
            {targetClientId ? targetClientName : (user ? `${user.firstName} ${user.lastName}` : "Athlete")} • ID: {targetClientId ? targetClientId.substring(0, 8) : (user?.email?.split('@')[0] || "Guest")}
          </p>
        </div>
      ...
      <Card className="border-muted bg-muted/20">
        <CardHeader className="pb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <CardTitle className="text-xl font-black">Weekly Strategy</CardTitle>
              <CardDescription className="text-xs font-bold uppercase tracking-widest text-muted-foreground">7-Day Protocol Overview</CardDescription>
            </div>
          </div>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3">
            {['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday'].map((day) => {
              ...
              const plan = weeklyPlans.find(p => p.day === day);
              const hasPlan = !!plan && (plan.workouts?.length > 0 || plan.meals?.length > 0);
              const workoutName = plan?.workouts?.[0]?.category || plan?.workouts?.[0]?.name || (hasPlan ? "Plan Assigned" : "Rest Day");
              ...
              return (
                <div 
                  ...
                  className={cn(
                    "p-4 rounded-2xl border transition-all flex flex-col items-center justify-center gap-2 cursor-pointer relative overflow-hidden group",
                    isActive ? "bg-primary text-primary-foreground border-primary shadow-lg scale-105" : "bg-card hover:border-primary/30",
                    !isActive && hasPlan ? "border-primary/20 bg-primary/5" : "",
                    !isActive && isToday ? "border-primary/50" : ""
                  )}
                >
                  <span className="text-[10px] font-black uppercase tracking-widest opacity-70">{day.substring(0, 3)}</span>
                  <div className="flex flex-col items-center gap-1">
                    <div className={cn(
                      "h-1.5 w-1.5 rounded-full",
                      isActive ? "bg-white" : (hasPlan ? "bg-primary" : "bg-muted-foreground/20")
                    )} />
                    <span className={cn(
                      "text-[9px] font-bold text-center line-clamp-1",
                      isActive ? "text-white/90" : "text-muted-foreground group-hover:text-primary transition-colors"
                    )}>
                      {workoutName}
                    </span>
                  </div>
        ...
        <Card className="col-span-4 shadow-xl shadow-primary/5">
          <CardHeader className="flex flex-row items-center justify-between border-b pb-6">
            <div>
              ...
              <CardTitle className="text-2xl font-black capitalize">
                {todayPlan?.day ? `${todayPlan.day}'s Strategy` : "Today's Protocol"}
              </CardTitle>
              <CardDescription className="font-medium">Assigned workouts for {todayPlan?.day || "today"}</CardDescription>
            </div>
          ...
          <CardContent className="pt-6">
            <div className="space-y-6">
              {todayPlan?.workouts?.length > 0 ? (
                todayPlan.workouts.map((workout: any, idx: number) => (
                  <WorkoutRow key={idx} workout={workout} />
                ))
              ) : (
                <div className="text-center py-10 text-muted-foreground font-bold italic">
                  No workouts assigned for today. Rest up!
                </div>
              )}
            </div>
          </CardContent>
            ...
            <div className="pt-6 border-t space-y-4">
              <div className="flex items-center justify-between">
                <p className="text-xs font-bold uppercase tracking-widest text-blue-500">
                  {todayPlan?.day ? `${todayPlan.day}'s Meals` : "Today's Meals"}
                </p>
                <span className="text-[10px] font-bold text-muted-foreground">{todayPlan?.meals?.length || 0} Assigned</span>
              </div>
                  ...
                  todayPlan.meals.map((meal: any, idx: number) => (
                    ...
                    >
                      <div className="h-10 w-10 rounded-xl bg-blue-500/10 flex items-center justify-center group-hover:bg-blue-500/20 transition-colors">
                        <HugeiconsIcon icon={Apple01Icon} className="h-4 w-4 text-blue-500" />
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-bold truncate group-hover:text-blue-500 transition-colors">{meal.name}</p>
                        <p className="text-[10px] text-muted-foreground font-medium">{meal.calories} kcal • {meal.protein}g protein</p>
                      </div>
                      <HugeiconsIcon icon={ArrowUpRight01Icon} className="h-3 w-3 text-muted-foreground opacity-0 group-hover:opacity-100 transition-all" />
                    </Link>
                  ))
                ) : (
                  <p className="text-xs font-bold text-muted-foreground italic text-center py-4">No specific meals assigned for today.</p>
                )}
              </div>
            </div>
