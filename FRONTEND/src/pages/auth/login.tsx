...
export default function LoginPage() {
  ...
  const onSubmit = async (data: LoginValues) => {
    ...
    try {
      ...
      const isAdminOrCoach = isAdminEmail(user.email) || user.role === RoleEnum.ADMIN || user.role === RoleEnum.COACH;
      setTimeout(() => navigate(isAdminOrCoach ? "/admin/workouts" : "/#philosophy"), 1000);
    } catch (error: any) {
  ...
  const handleGoogleSuccess = async (credentialResponse: any) => {
    ...
    try {
      ...
      const isAdminOrCoach = isAdminEmail(user.email) || user.role === 1 || user.role === 2;
      setTimeout(() => navigate(isAdminOrCoach ? "/admin/workouts" : "/#philosophy"), 1000);
    } catch (error: any) {
